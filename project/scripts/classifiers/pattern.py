...
class PatternMatcher:
    ...
    def match(self, rygb_image: np.ndarray) -> np.ndarray:
        ...
        # Padded shape for linear (non-circular) correlation
        padded_h = image_h + template_h - 1
        padded_w = image_w + template_w - 1
        padded_shape = (padded_h, padded_w)

        # Precompute template FFTs on first call (or if image size changed)
        if self._F_templates is None or self._padded_shape != padded_shape:
            self._precompute_ffts(padded_shape)

        # Normalize image globally: (H, W, 4) -> per-channel zero-mean unit-std
        image = rygb_image.astype(np.float32)
        for c in range(4):
            ch = image[:, :, c]
            image[:, :, c] = (ch - ch.mean()) / (ch.std() + 1e-8)

        # Compute image FFTs: one per channel
        # image_chw: (4, H, W)
        image_chw = image.transpose(2, 0, 1)
        F_image = np.fft.rfft2(image_chw, s=padded_shape)  # (4, pH, pW//2+1)

        # Accumulate scores across all angles, scales, channels
        scores = np.zeros(52)

        for angle_idx in range(len(self.angles)):
            for scale_idx in range(len(self.scales)):
                # F_templates: (52, 4, pH, pW//2+1)
                F_templates = self._F_templates[angle_idx][scale_idx]

                # Batched correlation: multiply conjugate of template FFT with image FFT
                # F_image broadcast: (1, 4, pH, pW//2+1)
                product = F_templates.conj() * F_image[None, :, :, :]  # (52, 4, pH, pW//2+1)

                # IRFFT back to spatial domain
                corr = np.fft.irfft2(product, s=padded_shape)  # (52, 4, pH, pW//2+1)

                # Max over spatial positions, mean over channels
                # corr shape: (52, 4, pH, pW)
                best = corr.reshape(52, 4, -1).max(axis=-1)  # (52, 4)
                scores += best.mean(axis=-1)                  # (52,)

        # Normalize by number of (angle, scale) hypotheses
        scores /= (len(self.angles) * len(self.scales))
