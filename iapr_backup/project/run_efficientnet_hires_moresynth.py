...
from project.scripts.dataset import PreprocessedDataset, Label
from project.scripts.synthesized_dataset import SynthesizedDataset
...
dataset = PreprocessedDataset(labels, target_size=HIRES, use_sobel=True)
...
synth_ds    = SynthesizedDataset(target_size=HIRES, use_sobel=True)
combined_ds = ConcatDataset([AugmentedDataset(train_ds, multiplier=5), AugmentedDataset(synth_ds, multiplier=20)])

train_loader = DataLoader(combined_ds, batch_size=8, shuffle=True, num_workers=4, pin_memory=True,
...
print(f"[hires {HIRES[0]}px moresynth] Real train: {n_train} (×5 aug) | Synthetic: {len(synth_ds)} (×20 aug) | Val: {n_val}")
...
model     = UnoEfficientNet(in_channels=5, variant="b0").to(device)
...
if __name__ == "__main__":
    train(
        ...
        use_focal     = True,
        center_weight = 0.1,
        save_path     = "best_model_640_moresynth.pth",
    )
