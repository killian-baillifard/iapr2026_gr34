...
dataset = PreprocessedDataset(labels, use_sobel=True)
...
synth_ds    = SynthesizedDataset(use_sobel=True)
combined_ds = ConcatDataset([AugmentedDataset(train_ds), AugmentedDataset(synth_ds, multiplier=2)])
...
def compute_pos_weight(dataset, num_cards=54):
    """Compute pos_weight from actual label frequencies: neg_count / pos_count per card."""
...
print(f"Real train: {n_train} (×5 aug) | Synthetic: {len(synth_ds)} (×2 aug) | Val: {n_val}")
...
if __name__ == "__main__":
    train(
        ...
        use_focal     = True,
        center_weight = 0.1,
        save_path     = "best_model_448_sobel.pth",
    )
