...
from project_crop_bad.scripts.dataset import Player, load_test_images_paths, TEST_IMAGES_PATH, CARD_LOOKUP
from project_crop_bad.scripts.dataset.synthesizer import synthesize_train_set
from project_crop_bad.scripts.preprocessing import preprocess
from project_crop_bad.scripts.preprocessing.sectors import slice_sectors
from project_crop_bad.scripts.preprocessing.cache import Cache, rebuild_cache
from project_crop_bad.scripts.token import detect_active_player
from project_crop_bad.scripts.classifiers.cnn import UNOCNNClassifier, train_model
from project_crop_bad.scripts.classifiers.pattern import PatternMatcher 
    ...
    with open(SUBMISSION_FILE, "w") as submission:
        ...
        for i, path in enumerate(paths):
            ...
            for j, sector in enumerate(slice_sectors(image)):
                ...
                scores = pattern_matcher.match(preprocessed)

                # For center card, keep only most likely card
                if j == 0:
                    center_card = CARD_LOOKUP[np.argmax(scores)]

                # For all players keep all probabilites above 95% quantile
                else:
                    indices, = np.where(scores > np.quantile(scores, 0.95))
                    
                    # Keep at most 4, prioritizing highest probabilities
                    if len(indices) > 4:
                        indices = indices[np.argsort(scores[indices])[::-1][:4]]
